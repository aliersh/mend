...
- **Foundry auto-loads `.env` from cwd.** To verify CI-equivalent behavior locally (no env), temporarily move `.env` aside (`mv .env .env.bak`); unsetting in the shell alone is insufficient because Foundry re-reads the file.